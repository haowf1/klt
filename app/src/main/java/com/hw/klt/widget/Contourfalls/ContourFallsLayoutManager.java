...
/**
 * Created by hao on 17-8-11.
 */

public class ContourFallsLayoutManager extends RecyclerView.LayoutManager{
    public static final String TAG = ContourFallsLayoutManager.class.getSimpleName();

    // Position of the header, which is the same value as its row. They can be used interchangeably
    static final int HEAD_POSTION = 0;
    ...
    private ContourSizeCalculator mSizeCalculator;

    public ContourFallsLayoutManager(ContourSizeCalculator.SizeCalculatorDelegate sizeCalculatorDelegate) {
        mSizeCalculator = new ContourSizeCalculator(sizeCalculatorDelegate);
    }
    ...
    public void onLayoutChildren(RecyclerView.Recycler recycler, RecyclerView.State state) {
        ...
        detachAndScrapAttachedViews(recycler);
        preFillGrid(ContourFallsLayoutManager.Direction.NONE, 0, initialTopOffset, recycler, state);
     ...
     */
    private int preFillGrid(ContourFallsLayoutManager.Direction direction, int dy, int emptyTop,
        ...
        while (nextPosition >= 0 && nextPosition < state.getItemCount()) {
            ...
            if (mIsFirstViewHeader && nextPosition == HEAD_POSTION) {
    ...
    private Size sizeForChildAtPosition(int position) {
        if (mIsFirstViewHeader && position == HEAD_POSTION) {
            return mHeaderViewSize;
        } else if (mIsFirstViewHeader && position > HEAD_POSTION) {
    ...
    private int firstChildPositionForRow(int row) {
        int offset = 0;
        if (mIsFirstViewHeader && row == HEAD_POSTION) {
            return HEAD_POSTION;
        } else if (mIsFirstViewHeader && row > HEAD_POSTION) {
    ...
    public int scrollVerticallyBy(int dy, RecyclerView.Recycler recycler, RecyclerView.State state) {
        ...
        if (dy > 0) {
            ...
            if (isLastChildVisible) {
                ...
                mFirstVisibleRow++;
                pixelsFilled = preFillGrid(ContourFallsLayoutManager.Direction.DOWN, Math.abs(dy), 0, recycler, state);
            ...
            if (mFirstVisibleRow == 0 && getDecoratedTop(topLeftView) - dy >= 0) {
                ...
                mFirstVisibleRow--;
                pixelsFilled = preFillGrid(ContourFallsLayoutManager.Direction.UP, Math.abs(dy), 0, recycler, state);
    ...
    public ContourSizeCalculator getSizeCalculator() {
