...
import com.hw.klt.dagger.Component.ApplicationComponent;
import com.hw.klt.dagger.Component.DaggerApplicationComponent;
...
import com.hw.klt.logcal.table.DaoMaster;
import com.hw.klt.logcal.table.DaoSession;
import com.hw.klt.rxbus.RxBus;
import com.hw.klt.util.CrashHandler;
import com.hw.klt.util.DownloadUtils;
import com.hw.klt.util.ToastUtils;
import com.hw.klt.util.device.DownloaderWrapper;
...
@DefaultLifeCycle(application = "com.hw.klt.ktlApplication",
        flags = ShareConstants.TINKER_ENABLE_ALL,
        loadVerifyFlag = false)
public class AndroidApplication extends DefaultApplicationLike {

    private static final String DB_NEWS_NAME = "news-db";
   ...
   //  因为下载那边需要用，这里在外面实例化在通过 ApplicationModule 设置
    ...
    //科大讯飞语音模块的初始化
    private void _initFmsg() {
        NlpManager.getmNlpManager().setNplManagerContext(getContext());
        SpeechUtility.createUtility(getContext(), SpeechConstant.APPID +"=59b7b4c1");
    }
    ...
    private void _initDatabase() {
        DaoMaster.DevOpenHelper helper = new DaoMaster.DevOpenHelper(getApplication(), DB_NEWS_NAME);
        Database database = helper.getWritableDb();
        mDaoSession = new DaoMaster(database).newSession();
        NewsTypeDao.updateLocalData(getApplication(), mDaoSession);
        MovieTypeDao.updateLocalData(getApplication(),mDaoSession);
